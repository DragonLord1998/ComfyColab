...
def ensure_pid_assets(
    backbone: str,
    force_redownload: bool = False,
) -> dict[str, str]:
    folder_paths = importlib.import_module("folder_paths")
    assets = selected_assets(backbone)
...
def filenames_by_role(backbone: str) -> dict[str, str]:
    return {
        role: specification["filename"]
        for role, specification in selected_assets(backbone).items()
    }
