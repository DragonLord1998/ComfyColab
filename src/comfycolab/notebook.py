...
def _bootstrap_display_source(port: int) -> str:
    return (
        ...
        "    if not _comfycolab_probe_proxy(proxy_url):\n"
        "        raise RuntimeError(\"proxy WebSocket readiness probe failed\")\n"
        ...
        "        proxy_error = iframe_error\n"
        "\n"
        "if proxy_error is not None:\n"
        "    print(f\"Colab proxy unavailable ({proxy_error}).\")\n"
        "\n"
        "if cloudflare_url is not None:\n"
        "    label = \"Cloudflare fallback URL\" if proxy_error is None else \"Selected Cloudflare URL\"\n"
        "    print(f\"\\n{label}:\")\n"
